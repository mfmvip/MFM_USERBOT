# for -<*>~ SOURCE TOKYO ~<*>- edit By: @MFMVIP
...
async def _(event):
    if event.fwd_from:
        return
    mentions = "𓆩 𝙎𝙤𝙪𝙧𝙘𝙚 𝙏𝙤𝙠𝙔𝙤 𝑮𝑹𝑶𝑼𝑷 𝑫𝑨𝑻𝑨 𓆪\n** ⪼ المشرفـون في ۿذه المجموعه :** \n"
    reply_message = None
...
async def get_users(show):
    if show.fwd_from:
        return
    mentions = "𓆩 𝙎𝙤𝙪𝙧𝙘𝙚 𝙏𝙤𝙠𝙔𝙤 𝑮𝑹𝑶𝑼𝑷 𝙏𝘼𝙂 𓆪\n**⪼ هييه تعالـو يحبابين**  𓎤: \n"
    reply_to_id = None
    if show.reply_to_msg_id:
        reply_to_id = show.reply_to_msg_id
    input_str = show.pattern_match.group(1)
...
async def _(event):
    ...
    async for i in bot.iter_participants(event.chat_id):
        ...
        if i.bot:
            ...
            n += 1
    if input_str:
        required_string = """𓆰 𝙎𝙤𝙪𝙧𝙘𝙚 𝙏𝙤𝙠𝙔𝙤  - 𝑮𝑹𝑼𝑶𝑷 𝑺𝑻𝑨𝑻𝑺 𓆪\n ٴ⊶─────≺ᴛᴏᴋʏᴏ≻─────⊷ 
...
async def fetch_info(chat, event):
    ...
    caption = "<b> 𓆰 𝙎𝙤𝙪𝙧𝙘𝙚 𝙏𝙤𝙠𝙔𝙤  - 𝑮𝑹𝑼𝑶𝑷 𝑫𝑨𝑻𝑨 𓆪\nٴ⊶─────≺ᴛᴏᴋʏᴏ≻─────⊷ </b>\n"
    ...
    if hasattr(chat_obj_info, "scam") and chat_obj_info.scam:
        ...
        caption = f"<b>𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝘿𝙀𝙑 - @MFMVIP 𓆪</b>"
    return caption
